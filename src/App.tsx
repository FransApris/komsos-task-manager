...
}> = ({ currentUser, tasksDb, notificationsDb }) => {
  const { attendances } = useData();

  useEffect(() => {
    // Hanya jalankan untuk USER biasa, bukan admin
    if (!currentUser || currentUser.role !== 'USER') return;

    const runChecks = () => {
      ReminderService.checkH1Reminders(currentUser, tasksDb, notificationsDb);
      ReminderService.checkAttendanceReminders(currentUser, tasksDb, notificationsDb, attendances);
    };

    runChecks(); // Langsung cek saat mount atau data berubah
    const interval = setInterval(runChecks, 30 * 60 * 1000); // Cek setiap 30 menit
    return () => clearInterval(interval);
  }, [currentUser, tasksDb, notificationsDb, attendances]);
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      case 'SPLASH':
        return <SplashScreen onFinish={() => handleNavigate(currentUser ? (currentUser.role?.startsWith('ADMIN_') || currentUser.role === 'SUPERADMIN' ? 'ADMIN_DASHBOARD' : 'NEW_TASK_ALERT') : 'LOGIN')} />;
      case 'LOGIN':
      ...
      case 'CHANGE_PASSWORD':
        return <ChangePassword onNavigate={handleNavigate} currentUser={currentUser} />;
      case 'HELP_CENTER':
