...
export const BarcodeScannerModal: React.FC<BarcodeScannerModalProps> = ({ onScan, onClose }) => {
    ...
    const startScanner = async () => {
      try {
        ...
        await html5QrCode.start(
          ...
          () => {
            // Success callback for scanning (not used here as we use the one above)
          }
        );
        setIsLoading(false);
      } catch (err: any) {
  ...
  const handleRetry = () => {
    setError(null);
    setIsLoading(true);
    // The useEffect will trigger again if we unmount/remount or we can manually call startScanner
    window.location.reload(); // Simple way for now if it fails hard
  };
